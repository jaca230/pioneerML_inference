...
void GroupSplitterRunner::Run(const RunOptions& options) {
  ...
  pioneerml::input_adapters::graph::GroupSplitterInputAdapter input_adapter;
  ...
  auto* inputs = dynamic_cast<pioneerml::GroupSplitterInputs*>(bundle.inputs.get());
  if (!inputs) {
    throw std::runtime_error("Failed to cast inputs to GroupSplitterInputs");
  }
  ...
  if (!output.isTensor()) {
    throw std::runtime_error("Group splitter TorchScript forward must return a tensor.");
  }
  ...
  pioneerml::output_adapters::graph::GroupSplitterOutputAdapter output_adapter;
