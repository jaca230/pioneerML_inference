...
void EndpointRegressorRunner::Run(const RunOptions& options) {
  ...
  pioneerml::input_adapters::graph::EndpointRegressorInputAdapter input_adapter;
  ...
  auto group_probs = ArrowToTensor(inputs->group_probs, {num_graphs, 3}, torch::kFloat32);
  ...
  int64_t graph_batch_size = 256;
  if (!config.is_null()) {
    graph_batch_size = ConfigInt64(config, "inference_graph_batch_size", 256);
  }
  ...
  auto preds_all =
      torch::zeros({num_graphs, 6}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCPU));
  ...
  if (options.check_accuracy) {
    ...
    targets = ArrowToFloatMatrix(inputs->y, num_graphs, 6);
  }
  ...
  for (int64_t graph_start = 0; graph_start < num_graphs; graph_start += graph_batch_size) {
    ...
    const int64_t graph_count = graph_end - graph_start;
    const int64_t group_count = group_end - group_start;
    if (group_count != graph_count) {
      throw std::runtime_error(
          "EndpointRegressor expects one group_probs row per graph for time-group inference.");
    }
    ...
    auto u_chunk = u.index({torch::indexing::Slice(graph_start, graph_end)});
    ...
    preds_all.index_put_({torch::indexing::Slice(graph_start, graph_end), torch::indexing::Slice()}, preds_chunk);

    if (options.check_accuracy) {
      auto target_chunk = targets.index({torch::indexing::Slice(graph_start, graph_end), torch::indexing::Slice()});
  ...
  pioneerml::output_adapters::graph::EndpointRegressorOutputAdapter output_adapter;
