...
void EventSplitterEventRunner::Run(const RunOptions& options) {
  ...
  pioneerml::input_adapters::graph::EventSplitterEventInputAdapter input_adapter;
  ...
  auto* inputs = dynamic_cast<pioneerml::EventSplitterEventInputs*>(bundle.inputs.get());
  if (!inputs) {
    throw std::runtime_error("Failed to cast inputs to EventSplitterEventInputs");
  }
  ...
  auto endpoint_preds = ArrowToTensor(inputs->endpoint_preds, {num_groups, 6}, torch::kFloat32);
  ...
  int64_t graph_batch_size = 64;
  if (!config.is_null()) {
    graph_batch_size = ConfigInt64(config, "inference_graph_batch_size", 64);
  }
  ...
  auto logits_all =
      torch::zeros({total_edges, 1}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCPU));

  torch::Tensor targets;
  if (options.check_accuracy) {
    if (!inputs->y_edge) {
      throw std::runtime_error("check_accuracy requested but y_edge targets are missing.");
    }
    targets = ArrowToFloatMatrix(inputs->y_edge, total_edges, 1);
  }
  ...
  for (int64_t graph_start = 0; graph_start < num_graphs; graph_start += graph_batch_size) {
    ...
    auto endpoint_preds_chunk = endpoint_preds.index({torch::indexing::Slice(group_start, group_end)});
    ...
    if (device.is_cuda()) {
      ...
      endpoint_preds_chunk = endpoint_preds_chunk.to(device);
    }
    ...
    std::vector<torch::jit::IValue> ivals = {
        ...
        endpoint_preds_chunk,
    };

    auto output = model.forward(ivals);
    if (!output.isTensor()) {
      throw std::runtime_error("Event splitter event TorchScript forward must return a tensor.");
    }

    auto logits_chunk = output.toTensor();
    if (device.is_cuda()) {
      logits_chunk = logits_chunk.to(torch::kCPU);
    }
    logits_chunk = logits_chunk.contiguous();
    logits_all.index_put_({torch::indexing::Slice(edge_start, edge_end), torch::indexing::Slice()}, logits_chunk);
  }

  auto edge_probs = torch::sigmoid(logits_all);
  auto edge_pred_arr = TensorToArrowFloat(edge_probs);

  if (options.check_accuracy) {
    const double threshold = options.threshold;
    auto preds_binary = (edge_probs >= threshold).to(torch::kFloat32);
    auto targets_f = targets.to(torch::kFloat32);
    const double accuracy =
        (preds_binary == targets_f).to(torch::kFloat32).mean().item<double>();
    const double exact_match =
        (preds_binary == targets_f).all(1).to(torch::kFloat32).mean().item<double>();

    auto truth = targets_f.select(1, 0);
    auto pred = preds_binary.select(1, 0);
    const auto tn = ((truth == 0) & (pred == 0)).sum().item<int64_t>();
    const auto fp = ((truth == 0) & (pred == 1)).sum().item<int64_t>();
    const auto fn = ((truth == 1) & (pred == 0)).sum().item<int64_t>();
    const auto tp = ((truth == 1) & (pred == 1)).sum().item<int64_t>();
    const double total = static_cast<double>(tp + fp + fn);

    nlohmann::json confusion = nlohmann::json::array();
    confusion.push_back({
        {"tp", total > 0.0 ? static_cast<double>(tp) / total : 0.0},
        {"fp", total > 0.0 ? static_cast<double>(fp) / total : 0.0},
        {"fn", total > 0.0 ? static_cast<double>(fn) / total : 0.0},
    });
  ...
  pioneerml::output_adapters::graph::EventSplitterEventOutputAdapter output_adapter;
