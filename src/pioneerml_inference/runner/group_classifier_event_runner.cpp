...
void GroupClassifierEventRunner::Run(const RunOptions& options) {
  ...
  pioneerml::input_adapters::graph::GroupClassifierEventInputAdapter input_adapter;
  ...
  int64_t num_groups = static_cast<int64_t>(inputs->num_groups);
  ...
  if (options.check_accuracy) {
    ...
    auto targets = ArrowToFloatMatrix(inputs->y, num_groups, 3);
  ...
  pioneerml::output_adapters::graph::GroupClassifierEventOutputAdapter output_adapter;
